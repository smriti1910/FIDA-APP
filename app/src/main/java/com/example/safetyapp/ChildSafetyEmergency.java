...
public class ChildSafetyEmergency extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_childsafety);

        Button childSafetyButton = findViewById(R.id.childSafetyButton);

        childSafetyButton.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                makeEmergencyCall("Child Safety");
            }
        });
    }
    ...
    private String getPhoneNumberForEmergencyType(String emergencyType) {
        ...
        switch (emergencyType) {
            case "Child Safety":
                phoneNumber = "9876543210"; // Replace with the actual child safety phone number
                break;
        }
        return phoneNumber;
    }
    ...
    public void onRequestPermissionsResult(int requestCode, String[] permissions, int[] grantResults) {
        ...
        if (requestCode == CALL_PERMISSION_REQUEST_CODE) {
            if (grantResults.length > 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                // Permission is granted, make the call
                String phoneNumber = getPhoneNumberForEmergencyType("Child Safety");
                if (phoneNumber != null) {
                    Intent callIntent = new Intent(Intent.ACTION_CALL);
                    callIntent.setData(Uri.parse("tel:" + phoneNumber));
                    startActivity(callIntent);
