...
public class TheftEmergencyActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_theft_emergency);

        Button theftRobberyButton = findViewById(R.id.theftRobberyButton);

        theftRobberyButton.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                makeEmergencyCall("Theft / Robbery");
            }
        });
    }
    ...
    private String getPhoneNumberForEmergencyType(String emergencyType) {
        ...
        switch (emergencyType) {
            case "Theft / Robbery":
                phoneNumber = "5555555555"; // Replace with the actual theft / robbery phone number
                break;
        }
        return phoneNumber;
    }
    ...
    private void reportToPoliceStation() {
        ...
        // Get the current location dynamically
        // For this example, let's assume the location is hardcoded for demonstration purposes
    ...
    public void onRequestPermissionsResult(int requestCode, String[] permissions, int[] grantResults) {
        ...
        if (requestCode == CALL_PERMISSION_REQUEST_CODE) {
            if (grantResults.length > 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                // Permission is granted, make the call
                String phoneNumber = getPhoneNumberForEmergencyType("Theft / Robbery");
                if (phoneNumber != null) {
                    Intent callIntent = new Intent(Intent.ACTION_CALL);
                    callIntent.setData(Uri.parse("tel:" + phoneNumber));
                    startActivity(callIntent);
